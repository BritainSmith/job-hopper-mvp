...
export class DataCleaningService {
  ...
  cleanJobData(job: PrismaJob): CleanedJobData {
    const cleaned: CleanedJobData = {
      ...
      remoteType: this.extractRemoteType(job.title, job.location, job.tags || ''),
   ...
   */
  private parseSalary(salary?: string | null): { min: number; max: number; currency: string } | null {
    if (!salary) return null;

    try {
      // Common salary patterns
      const patterns = [
        /\$(\d{1,3}(?:,\d{3})*)\s*-\s*\$(\d{1,3}(?:,\d{3})*)/i, // $50,000 - $80,000
        /(\d{1,3}(?:,\d{3})*)\s*-\s*(\d{1,3}(?:,\d{3})*)\s*(?:usd|dollars?)/i, // 50,000 - 80,000 USD
        /\$(\d{1,3}(?:,\d{3})*)\s*to\s*\$(\d{1,3}(?:,\d{3})*)/i, // $50,000 to $80,000
      ];
      ...
      this.logger.warn('Failed to parse salary:', { salary, error });
      return null;
    }
  }
   ...
   */
  private extractExperienceLevel(title: string, tags: string): 'entry' | 'mid' | 'senior' | 'lead' | 'unknown' {
   ...
   */
  private extractJobType(title: string, tags: string): 'full-time' | 'part-time' | 'contract' | 'internship' | 'unknown' {
    const text = `${title} ${tags}`.toLowerCase();
   ...
   */
  private extractRemoteType(title: string, location: string, tags: string): 'remote' | 'hybrid' | 'onsite' | 'unknown' {
  ...
  async getDataQualityMetrics(jobs: PrismaJob[]): Promise<DataQualityMetrics> {
    ...
    for (const job of jobs) {
      ...
      if (job.salary) {
        ...
        issues.set('No salary information', (issues.get('No salary information') || 0) + 1);
      }
    ...
    const averageQualityScore = totalJobs > 0 ? totalQualityScore / totalJobs : 0;
