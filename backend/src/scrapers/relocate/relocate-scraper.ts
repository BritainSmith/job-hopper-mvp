...
export class RelocateScraper extends BaseScraper {
  ...
  async scrapeJobs(options?: ScrapingOptions): Promise<Job[]> {
    ...
    this.logger.log(`Starting Relocate.me scraping (version: ${this.currentVersion})`);
    ...
    try {
      ...
      if (detectedVersion && detectedVersion !== this.currentVersion) {
        ...
        this.logger.log(`Relocate.me structure changed, switching to ${detectedVersion}`);
    ...
    this.logger.log(`Successfully scraped ${allJobs.length} jobs from Relocate.me`);
