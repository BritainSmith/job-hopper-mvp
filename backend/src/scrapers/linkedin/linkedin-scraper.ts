...
export class LinkedInScraper extends BaseScraper {
  ...
  async scrapeJobs(options?: ScrapingOptions): Promise<Job[]> {
    ...
    this.logger.log(`Starting LinkedIn scraping (version: ${this.currentVersion})`);
    ...
    try {
      ...
      if (detectedVersion && detectedVersion !== this.currentVersion) {
        ...
        this.logger.log(`LinkedIn structure changed, switching to ${detectedVersion}`);
    ...
    this.logger.log(`Successfully scraped ${allJobs.length} jobs from LinkedIn`);
  ...
  private async detectVersion(): Promise<string | null> {
    try {
      ...
      if (html.includes('job-card-container') || html.includes('job-card-list')) {
        return 'v2';
      }
      
      // Check for v1 indicators
      if (html.includes('job-search-card') || html.includes('artdeco-pagination')) {
        return 'v1';
      }
