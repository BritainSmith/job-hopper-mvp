...
describe('ArbeitnowV1Parser', () => {
  ...
  describe('parseDate', () => {
    it('should parse ISO date', () => {
      const date = (parser as any).parseDate('2024-01-01');
      expect(date).toBeInstanceOf(Date);
      expect(date.getUTCFullYear()).toBe(2024);
    });
    it('should parse relative date', () => {
      const now = new Date();
      const date = (parser as any).parseDate('2 days ago');
      expect(date).toBeInstanceOf(Date);
      expect(date.getUTCDate()).toBe(now.getUTCDate() - 2);
    });
    it('should parse German date', () => {
      const date = (parser as any).parseDate('01.02.2023');
      expect(date).toBeInstanceOf(Date);
    ...
    it('should handle error and return new Date', () => {
      const date = (parser as any).parseDate(undefined);
      expect(date).toBeInstanceOf(Date);
    });
  });

  describe('parseRelativeDate', () => {
    it('should parse days, hours, minutes, seconds', () => {
      const now = new Date();
      const parserAny = parser as any;
      expect(parserAny.parseRelativeDate('3 days ago').getUTCDate()).toBe(now.getUTCDate() - 3);
      expect(parserAny.parseRelativeDate('2 hours ago')).toBeInstanceOf(Date);
      expect(parserAny.parseRelativeDate('15 minutes ago')).toBeInstanceOf(Date);
      expect(parserAny.parseRelativeDate('30 seconds ago')).toBeInstanceOf(Date);
    });
    it('should return now for unrecognized format', () => {
      const now = new Date();
      const date = (parser as any).parseRelativeDate('someday');
  ...
  describe('parseGermanDate', () => {
    it('should parse DD.MM.YYYY', () => {
      const date = (parser as any).parseGermanDate('01.02.2023');
      expect(date).toBeInstanceOf(Date);
      expect(date.getUTCFullYear()).toBe(2023);
      expect(date.getUTCMonth()).toBe(1); // February (zero-based)
      expect(date.getUTCDate()).toBe(1);
    ...
    it('should parse DD.MM.YY', () => {
      const date = (parser as any).parseGermanDate('01.02.23');
      expect(date).toBeInstanceOf(Date);
      expect(date.getUTCFullYear()).toBe(2023);
    });
    it('should return null for invalid', () => {
      expect((parser as any).parseGermanDate('invalid')).toBeNull();
    });
  });
